...
function renderDbTable() {
  ...
  const sorted = sortDbMembers(custom);
  const rows = sorted.map((m, i) => {
    ...
    return `
      <div class="db-row">
        ...
        <div class="db-nama">
          <div class="db-full">${esc(member.fullname)}</div>
        ...
        <div class="db-actions">
          ...
          <button class="btn btn-danger db-masuk" onclick="adminDeleteCustomMember('${member.id}')">Hapus</button>
        </div>
      </div>`;
  }).join("");
  ...
  table.innerHTML = `
    <div class="db-toolbar">
      <div>
        <h2 class="section-title" style="margin-bottom:.2rem"><span class="dot"></span> Database Akun Anggota</h2>
        <p style="color:var(--muted);font-size:.85rem">Akun hanya dibuat oleh admin. Password = ID + "123" (bebas huruf besar/kecil). Buat akun baru lewat tab <b>Tambah Anggota</b>.</p>
      </div>
    ...
    <div class="db-count">${sorted.length} akun</div>
    ...
    <div class="db-list">${rows || '<div class="empty">Tidak ada akun yang cocok. Gunakan tab Tambah Anggota untuk membuat akun baru.</div>'}</div>`;
}
...
function adminDeleteCustomMember(memberId) {
  const member = getMemberById(memberId);
  confirmAction({
    title: "Hapus anggota buatan admin?",
    message: 'Anggota "' + (member ? member.name : memberId) + '" beserta akunnya akan dihapus.',
    confirmText: "Hapus",
    onConfirm: () => {
      saveCustomMembers(getCustomMembers().filter((m) => m.id !== memberId));
      localStorage.removeItem(LS_MSG_PREFIX + memberId);
